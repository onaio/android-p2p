...
class WifiDirectDataSharingStrategyTest : RobolectricTest() {
  ...
  fun setUp() {
    ...
    wifiP2pChannel = mockk(relaxed = true)
    wifiP2pReceiver = mockk(relaxed = true)
    wifiP2pInfo = mockk(relaxed = true)
    wifiP2pGroup = mockk(relaxed = true)
    socket = mockk(relaxed = true)
    context = spyk(Activity())
    onDeviceFound = mockk(relaxed = true)
    pairingListener = mockk(relaxed = true)
    operationListener = mockk(relaxed = true)
    payloadReceiptListener = mockk(relaxed = true)
    device = mockk()
    syncPayload = mockk()
    coroutineScope = TestCoroutineScope(TestCoroutineDispatcher())
    onSocketConnectionMade = mockk()
    expectedManifest = populateManifest()
    dataOutputStream = mockk(relaxed = true)
